...
export abstract class ARFeature {
    ...
    protected _handler: ARHandler<ARAnchor> | null = null;
    public setHandler (handler: ARHandler<ARAnchor> | null) {
    ...
    public start () {
        if (this._config && this._config.enable) {
            this.onEnable();
        } else {
            this.onDisable();
        }
...
export abstract class ARTrackingFeature<T extends ARAnchor> extends ARFeature {
    ...
    public onAddTracking (data: T[]) {
        if (data && data.length > 0) {
            this.onAddEvent.trigger(data);
        }
    }

    public onUpdateTracking (data: T[]) {
        if (data && data.length > 0) {
            this.onUpdateEvent.trigger(data);
        }
    }

    public onRemoveTracking (data: T[]) {
        if (data && data.length > 0) {
            this.onRemoveEvent.trigger(data);
        }
    }
}
