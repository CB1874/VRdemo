...
import { ARPlane } from '../../../../../ar/component/framework/utils/ar-defines';
...
export class ARMobileHandlerPlaneDetection extends ARHandlerPlaneDetection {
    ...
    public update () {
        ...
        let planeInfos = device.NativeObj.getRemovedPlanesInfo();
        if (planeInfos.length > 0) {
            this.assembleInfos(planeInfos, planes);
            if (planes.length > 0) {
                this._feature.onRemoveTracking(planes);
            }
        }

        planeInfos = device.NativeObj.getAddedPlanesInfo();
        if (planeInfos.length > 0) {
            planes.length = 0;
            this.assembleInfos(planeInfos, planes);
            if (planes.length > 0) {
                this._feature.onAddTracking(planes);
            }
        }

        planeInfos = device.NativeObj.getUpdatedPlanesInfo();
        if (planeInfos.length > 0) {
            planes.length = 0;
            this.assembleInfos(planeInfos, planes);
            if (planes.length > 0) {
                this._feature.onUpdateTracking(planes);
            }
        }
    }
