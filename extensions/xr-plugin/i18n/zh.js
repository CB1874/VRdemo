module.exports = {
    ...
    action: {
        ...
        fallback: {
            displayMethod: 'AR降级效果的表现形式。',
            distance: '虚拟内容和摄像机的距离。',
            fallBackScene: '降级时要跳转的场景名称。',
        },
    },
    factor: {
        plane_direction: {
    ...
    tracking: {
        trackingType: '追踪类型',
        enableFallBack: '控制native/web端是否启用降级，跳过Factor并且直接执行Action中的Fallback。',
        enableNativeFallBack: '控制native端是否启用第三方库执行图像识别，以及是否执行Fallback action。',
        enableWebFallBack: '控制web端是否启用第三方库执行图像追踪。',
        ...
        fallback: '此行为在不支持AR能力的设备上执行，可控制虚拟内容的展示和摆放方式',
    ...
    screen_touch_gesture: {
        ...
        placeAction: {
            ...
            distance: '设置交互物摆放位置与交互器的距离。',
            detectOcclusion: '设置交互物摆放是否受遮挡。',
            located_Prefab: '用于定位虚拟物体的摆放位置的预制体。',
    ...
    builder: {
        ...
        foveation_level: '调整注视点渲染等级，较高的级别将导致较低的GPU负载，但眼睛渲染纹理的边缘会出现模糊',
    },
    videoplayer: {
    ...
    webview: {
        url: '网页 URL',
        content: '关联带有MeshRenderer组件的WebViewContent作为网页材质渲染对象',
    },
    spatialAudio: {
        ...
        distanceRolloffModel: '表示不同的距离效应衰减模型',
        minDistance: '距离衰减中最小距离',
        maxDistance: '距离衰减中最大距离',
    },
    compositionLayer: {
        rtcamera: '渲染相机，其可见内容会渲染到合成层中',
        rtwidth: '渲染相机中渲染目标纹理的宽度',
        rtheight: '渲染相机中渲染目标纹理的高度',
        layertype: '合成层类型：Underlay和Overlay',
        shapetype: '合成层的形状，如平面，曲面等',
        locktype: '合成层锁定类型：WorldLock（不跟随头部转动）和HeadLock（跟随头部转动）',
        depth: '合成层深度值：(Underlay)2/1/0 [EyeBuffer] (Overlay)0/1/2',
        cylinderradius: '曲面半径',
        cylindercentralangle: '曲面中心角',        
        texturetype: '合成层纹理类型：动态和静态',
        staticimage: '静态纹理资源',
    },
    passthrough: {
        opacity: '不透明度在0.0到1.0范围内',
    },
};
