...
const _commonjsHelpers = require('./_commonjsHelpers.a2621663.js');
...
const options = require('./options.8a4ee5c8.js');
...
if (!_commonjsHelpers.commonjsGlobal[gracefulQueue]) {
  publishQueue(_commonjsHelpers.commonjsGlobal, fs$h[gracefulQueue]);
}
...
async function afterBundleInit(plateform, options$1) {
  const android = await options.generateOptions(plateform, options$1);
  options$1.packages[plateform] = android;
  const renderBackEnd = android.renderBackEnd;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl1 = renderBackEnd.gles2 ?? true;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl3 = renderBackEnd.gles3 ?? true;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl4 = renderBackEnd.vulkan ?? true;
  options$1.buildScriptParam.platform = "ANDROID";
}
async function afterInit(plateform, options$1, result, cache) {
  const android = await options.generateOptions(plateform, options$1);
  options$1.packages[plateform] = android;
  const renderBackEnd = android.renderBackEnd;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl1 = renderBackEnd.gles2 ?? true;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl3 = renderBackEnd.gles3 ?? true;
  options$1.assetSerializeOptions["cc.EffectAsset"].glsl4 = renderBackEnd.vulkan ?? true;
  if (options.semver.lt(Editor.App.version, "3.8.0")) {
    const staticsInfo = {
    ...
    result.staticsInfo["B100005" /* packageName */] = android.packageName;
    result.staticsInfo["B100011" /* orientation */] = android.orientation;
  }
  const res = await options.checkAndroidAPILevels(plateform, android.apiLevel, options$1);
  if (res.error) {
  ...
  Editor.Metrics.trackEvent({
    ...
    label: "Vulkan",
    value: options$1.packages[plateform].renderBackEnd.vulkan ? 100 : 0
  });
  ...
  const params = options$1.cocosParams ? options$1.cocosParams : result.compileOptions;
  ...
  if (android.remotePreView) {
    params.cMakeConfig.USE_XR_REMOTE_PREVIEW = "set(USE_XR_REMOTE_PREVIEW ON)";
  } else {
    params.cMakeConfig.USE_XR_REMOTE_PREVIEW = "set(USE_XR_REMOTE_PREVIEW OFF)";
  }
  params.cMakeConfig.CC_ENABLE_SWAPPY = !!android.swappy;
  params.platform = "xr";
  params.platformName = plateform;
  lib.writeFileSync(result.paths.compileConfig, JSON.stringify(params));
  await getNativePackToolMg(params.enginePath, plateform);
  nativePackToolManager.init(params);
}
function replaceEnvVariables(str) {
  return str.replace(/\$\{([^}]*)\}/g, (_, n) => process.env[n] === void 0 ? _ : process.env[n]).replace(/(~)/g, (_, n) => process.env.HOME);
}
async function replaceInFile(patterns, filepath) {
  filepath = replaceEnvVariables(filepath);
  if (!lib.existsSync(filepath)) {
    console.log(`While replace template content, file ${filepath}`);
    return;
  }
  const lines = (await lib.readFile(filepath)).toString("utf8").split("\n");
  const newContent = lines.map((l) => {
    patterns.forEach((p) => {
      l = l.replace(new RegExp(p.reg), replaceEnvVariables(p.text));
    });
    return l;
  }).join("\n");
  await lib.writeFile(filepath, newContent);
}
async function getNativePackToolMg(enginePath = "", plateform) {
  ...
  class XrPackTool extends AndroidPackTool {
    ...
    resetPath() {
      if (options.semver.lt(Editor.App.version, "3.7.0")) {
        this.paths.platformTemplateDirName = plateform;
      }
      this.setEnv("NATIVE_DIR", this.paths.platformTemplateDirInPrj);
    }
    async modifyAndroidSettingsGradle() {
      const XR_COMMON_PATH = "../../../extensions/xr-plugin/common";
      await replaceInFile([
        { reg: "^project\\(':libxrcommon'\\)\\.projectDir.*", text: `project(':libxrcommon').projectDir  = new File('${XR_COMMON_PATH}','cocos/platform/android/libcocosxr')` }
      ], require$$1.join(this.paths.nativePrjDir, "settings.gradle"));
    }
    async create() {
      this.resetPath();
      ...
      await this.modifyAndroidSettingsGradle();
      await this.configAndroidInstant();
      if (typeof this.generateAppNameValues !== "undefined") {
...
async function beforeCompressSettings(plateform, options, result, cache) {
  ...
  result.settings.remotePreView = options.packages[plateform].remotePreView;
  result.settings.foveationLevel = options.packages[plateform].foveationLevel;
}
async function afterCompressSettings(plateform, options, result, cache) {
  ...
  content.rendering = {
    ...
    remotePreView: options.packages[plateform].remotePreView,
    foveationLevel: options.packages[plateform].foveationLevel
  };
...
async function onAfterInit(options, result, cache) {
  const params = options.cocosParams ? options.cocosParams : result.compileOptions;
  params.cMakeConfig.USE_XR = "set(USE_XR ON)";
  params.cMakeConfig.XR_OEM_HUAWEI = "set(XR_OEM_SNAPDRAGON_SPACES ON)";
  await afterInit("xr-spaces", options, result, cache);
}
async function onBeforeCompressSettings(options, result, cache) {
  await beforeCompressSettings("xr-spaces", options, result);
}
async function onAfterCompressSettings(options, result, cache) {
  await afterCompressSettings("xr-spaces", options, result);
}
async function onAfterBuild() {
  await afterBuild();
}
async function onAfterBundleInit(options) {
  await afterBundleInit("xr-spaces", options);
}
